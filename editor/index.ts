...
const Editor = (function () {
        ...
        if ((confirm != null ? confirm.toLowerCase() : void 0) === "yes") {
            ...
            this.sendLayout(this.editor.getValue())
                ...
                .catch(err => {
                    console.log(`err`, err)
                    alert(`
                    Sorry, there was an error sending your code to the server. 
    ...
    App.prototype.sendLayout = async function (code) {
        ...
        const playerName = localStorage["name"];
        if (!playerName) {
            alert("Please rewrite your name");
            return;
        }
    ...
    App.prototype.onChange = function (e) {
        ...
        insertTextAction = e.action === "insert";
        if (insertTextAction) {
            this.increaseStreak();
            this.debouncedEndStreak();
        }
        this.throttledShake();
        pos = insertTextAction ? e.end : e.start;
